...
export async function fetchLoadDetail(id: string, scope?: 'leg' | 'trip'): Promise<any> {
  const q = scope ? `?alternatives=1&scope=${scope}` : '';
  return apiGet<any>(`/api/public/driver/loads/${id}${q}`);
}
